...
define("tradershub.MainFlow.Endpoint.mvc$controller.SwitchOnChange.UpdateFeatureFlagByNameJS", [], function() {
    return function($parameters, $actions, $roles, $public) {
        let featureFlags = localStorage.featureFlags

        if (featureFlags) {
            featureFlags = JSON.parse(featureFlags)

            featureFlags?.forEach(flag => flag.name === $parameters.FlagName && (flag.enabled = !flag.enabled))

            localStorage.featureFlags = JSON.stringify(featureFlags)
        }
